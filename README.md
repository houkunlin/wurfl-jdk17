...
- **HarmonyOS 支持**：新增 `HarmonyOSMatcher`，支持纯 HarmonyOS 格式 UA（不含 `Android`
  ...
  - `getAdvertisedDeviceOs()` 对 HarmonyOS UA 返回 `"HarmonyOS"` 而非 `"Android"`
  - `isAndroid()` 对数据文件或 UA 解析出 HarmonyOS 的设备返回 `false`
  - `knownOsNames` 新增 `"HarmonyOS"`，防止被 `tryFallbackOs()` 误判为 `"Linux"`
...
- **安全增强**：
  ...
  - **CRLF 日志注入防护**：`StringMatchUtils.sanitizeForLog()` 替换 `\r`/`\n` 为可视符号 `␍`/`␊`，
    在缓存提供者和正常化器中对所有用户可控 UA 进行日志前消毒
...
- **线程安全与锁统一**：
  ...
  - `AbstractMatcher.filter` 字段加 `volatile` + 双重检查锁定，防止并发创建设备索引
- **设备名称增强**：
  - **通用设备兜底名称**：`DeviceName` / `CompleteDeviceName` 在品牌和型号都为空时，
    以 "浏览器 on OS" 作为兜底名称（如 `"UC Browser on Android"`）
  - **Generic 设备版本替换**：名称中的老旧 OS 版本号替换为 UA 解析的实际版本
    （如 `"Generic Android 1.5 Tablet"` → `"Generic Android 14 Tablet"`）
...
dependencies {
  implementation 'com.houkunlin:wurfl-jdk17:1.2.0'
}
```
